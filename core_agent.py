...
class VisualTelecomAnalyst:
    def __init__(self):
        ...
        # 修复后的 YAML 读取逻辑
        # ==========================================
        yaml_path = os.path.join(BASE_DIR, "schema.yaml") # 强制使用标准后缀
        
        # 如果标准后缀找不到，再尝试找你以前可能写错的 1.ymal 或 schema.ymal
        if not os.path.exists(yaml_path):
             yaml_path = os.path.join(BASE_DIR, "1.ymal")
        if not os.path.exists(yaml_path):
             yaml_path = os.path.join(BASE_DIR, "schema.ymal")
        ...
        try:
            ...
            print(f"❌ YAML 文件解析失败，请检查语法（如是否有乱加的逗号）: {e}")
            self.config = {}
            self.golden_sqls = []
    ...
    def run_workflow(self, user_query, history=[]):
        ...
        system_prompt = f"""
            ...
            - 第二行输出图表类型：
              ...
              `CHART: dual_axis` (双轴图)。
              不需要画图则输出 `CHART: none`。            
            - 第三行输出极简图表标题（15字以内），格式为 `TITLE: <标题>`。图例如含有“率”“比”“%”，则注意修改为百分比数字。
            - 第四行：COMMENT: 数据来源：<引用的核心表名> | 时间范围：<提取的查询时间跨度>
